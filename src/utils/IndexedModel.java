...
public class IndexedModel
{
	...
	public void CalcNormals()
	{
		for(int i = 0; i < m_indices.size(); i += 3)
		{
			...
			Vector4f v1 = m_positions.get(i1).copy().Sub(m_positions.get(i0));
			Vector4f v2 = m_positions.get(i2).copy().Sub(m_positions.get(i0));

			Vector4f normal = v1.Cross(v2).Normalized();

			m_normals.set(i0, m_normals.get(i0).copy().Add(normal));
			m_normals.set(i1, m_normals.get(i1).copy().Add(normal));
			m_normals.set(i2, m_normals.get(i2).copy().Add(normal));
		}
	...
	public void CalcTangents()
	{
		for(int i = 0; i < m_indices.size(); i += 3)
		{
			...
			Vector4f edge1 = m_positions.get(i1).copy().Sub(m_positions.get(i0));
			Vector4f edge2 = m_positions.get(i2).copy().Sub(m_positions.get(i0));
			...
			m_tangents.set(i0, m_tangents.get(i0).copy().Add(tangent));
			m_tangents.set(i1, m_tangents.get(i1).copy().Add(tangent));
			m_tangents.set(i2, m_tangents.get(i2).copy().Add(tangent));
		}

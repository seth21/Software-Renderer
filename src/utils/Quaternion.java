...
public class Quaternion
{
	...
	public Quaternion(Vector4f axis, float angleRads)
	{
		float sinHalfAngle = (float)Math.sin(angleRads / 2);
		float cosHalfAngle = (float)Math.cos(angleRads / 2);
	...
	public Quaternion(Quaternion quaternion){
		this(quaternion.m_x, quaternion.m_y, quaternion.m_z, quaternion.m_w);
	}
	...
	public Quaternion Normalize()
	{
		...
		return set(m_x / length, m_y / length, m_z / length, m_w / length);
	}

	public Quaternion Conjugate()
	{
		return set(-m_x, -m_y, -m_z, m_w);
	}

	public Quaternion Mul(float r)
	{
		return set(m_x * r, m_y * r, m_z * r, m_w * r);
	}
	...
	{
		return set(m_x - r.GetX(), m_y - r.GetY(), m_z - r.GetZ(), m_w - r.GetW());
	}
	...
	public Quaternion NLerp(Quaternion dest, float lerpFactor, boolean shortest)
	{
		...
		return correctedDest.copy().Sub(this).Mul(lerpFactor).Add(this).Normalize();
	}
	...
	public Quaternion SLerp(Quaternion dest, float lerpFactor, boolean shortest)
	{
		...
		return this.copy().Mul(srcFactor).Add(correctedDest.copy().Mul(destFactor));
	}

...
public class Camera2 extends Observer
{

	public static Camera2 createPerspectiveCamera(float FOVDegrees, int viewportWidth, int viewportHeight, float nearPlane, float farPlane, Input input){
		Matrix4f projection = new Matrix4f().InitPerspective((float) Math.toRadians(FOVDegrees), 
				(float)viewportWidth/(float)viewportHeight, 
				nearPlane, 
				farPlane);
		return new Camera2(projection, viewportWidth, viewportHeight);
	}
	
	public static Camera2 createOrthoCamera(float left, float right, float top, float bottom, float nearPlane, float farPlane, Input input){
		Matrix4f projection = new Matrix4f().InitOrthographic(left, right, bottom, top,
				nearPlane, 
				farPlane);
		return new Camera2(projection, 0, 0);
	}
	
	public Camera2(Matrix4f projection, int viewportWidth, int viewportHeight)
	{
